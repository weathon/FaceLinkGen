...
def drop_50_percent_blocks_bchw(x):
    B, C, H, W = x.shape
    bh, bw = H // 8, W // 8
    Ht, Wt = bh * 8, bw * 8
    x = x[:, :, :Ht, :Wt].view(B, C, 8, bh, 8, bw)
    x = x.permute(0, 2, 4, 1, 3, 5).reshape(B, 64, C, bh, bw)
    
    # Generate random indices for each image in batch
    rand = torch.rand(B, 64, device=x.device)
    idx = rand.topk(64//2, dim=1).indices
    
    # Gather selected patches
    batch_idx = torch.arange(B, device=x.device).view(-1, 1)
    x = x[batch_idx, idx] # Shape: (B, 32, C, bh, bw)
    
    # Reconstruct with spatial layout (filling dropped parts with zeros or reshaping)
    # To maintain B, C, H, W shape with 50% patches removed (zeroed):
    out = torch.zeros(B, 64, C, bh, bw, device=x.device)
    out[batch_idx, idx] = x
    out = out.view(B, 8, 8, C, bh, bw).permute(0, 3, 1, 4, 2, 5).reshape(B, C, Ht, Wt)
    return out
...
class FaceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # c_img = shuffle_into_64_blocks(c_img)
        # try:
...
val_dataset = FaceDataset(val_paths[:20])  
...
optimizer = torch.optim.AdamW(student.parameters(), lr=2e-4, weight_decay=1e-2)
...
for e in range(epochs):
    ...
    for fn, embedding_teacher, conv_raw in tqdm.tqdm(loader): 
        ...
        conv_raw = drop_50_percent_blocks_bchw(conv_raw)
