...
os.environ["CUDA_VISIBLE_DEVICES"] = os.environ.get("FORCE_GPU", "0")
import sys
...
def convert_batch(conv_raw):
    ...
    inputs, _ = util.form_training_batch(conv_raw, [1] * conv_raw.shape[0])
    imgs = inputs.reshape(inputs.shape[0], -1, 112, 112).mean(1, keepdim=True).repeat(1, 3, 1, 1)
    imgs = imgs.float()
...
if has_teacher and count > 0:
    print(f"RESULT partial cosine_loss={total_cos/count:.4f} triplet={total_trip/count:.4f} mae={total_mae/count:.4f} n_batches={count}")
