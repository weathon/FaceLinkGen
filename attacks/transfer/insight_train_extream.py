...
def fft_high_pass(img, cutoff=40, order=4):
    if img.ndim == 2:
...
class FaceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_s = Image.open(p).resize((112,112)).convert("L").convert("RGB")
        # img_s = Image.fromarray((img_s * 255).astype("uint8")).resize((112,112)).convert("L").convert("RGB")
        # img_s = Image.open(p).convert("RGB")

        img_s = gaussian_high_pass(np.array(img_s), 0.5)
        c_img = torch.tensor(np.array(img_s)).permute(2,0,1).float()
        c_img = (c_img - c_img.min()) / (c_img.max() - c_img.min() + 1e-8)
...
for i in tqdm.tqdm(range(len(val_paths)), desc="Preloading cache_templates"):
    p = val_paths[i]
    # img_s = DeepFace.extract_faces(p, detector_backend = "opencv", enforce_detection=False)[0]["face"]
    img_s = Image.open(p).resize((112,112))
...
loader = DataLoader(dataset, batch_size=64, shuffle=True, num_workers=64, pin_memory=True)
val_loader = DataLoader(val_dataset, batch_size=64, shuffle=True, num_workers=32, pin_memory=True)
...
for e in range(epochs):
    ...
    for fn, embedding_teacher, conv_raw in tqdm.tqdm(loader): 
        ...
        if count % 50 == 0:
            ...
            print("Validation Cosine:", total_cos/count)
            student = student.train()
