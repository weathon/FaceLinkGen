    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            if random.random() < 0.3:
                blur_sigma = random.uniform(0.4, 1.6)
                c_img = TF.gaussian_blur(c_img, (_auto_kernel_from_sigma(blur_sigma),) * 2, sigma=[blur_sigma, blur_sigma])
            hp_strength = random.uniform(0.05, 3.0)
            hp_sigma = random.uniform(1.0, 4.0)
            c_img = highpass(c_img, strength=hp_strength, kernel_size=None, sigma=hp_sigma)
            if random.random() < 0.5:
                c_img = highpass(c_img, strength=hp_strength, kernel_size=None, sigma=hp_sigma)
            # c_img = bw_aug(c_img)
...
with torch.no_grad():
    tr_fn, tr_emb, tr_raw = next(iter(loader))
    tr_img = convert_batch(tr_raw)
    tr_img = zscore_batch(tr_img)
    tr_stats = (
        tr_img.min().item(),
        tr_img.max().item(),
        tr_img.mean().item(),
    ...
    va_fn, va_emb, va_raw = next(iter(val_loader))
    va_raw = highpass(va_raw)
    va_img = convert_batch(va_raw)
    va_img = zscore_batch(va_img)
    va_stats = (
        va_img.min().item(),
        va_img.max().item(),
        va_img.mean().item(),
    ...
    print("Sanity check (train) min/max/mean/std:", tr_stats)
    print("Sanity check (val)   min/max/mean/std:", va_stats)
