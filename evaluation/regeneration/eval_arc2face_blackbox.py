...
def compare_face_amazon(img1, img2):
    ...
    while True:
        try:
            ...
            print(result)
            amazon_result = result.get("amazon", {})
            amazon_error = amazon_result.get("error", {})
            error_message = amazon_error.get("message", "")
            if (
                amazon_result.get("status") == "fail"
                and "InvalidParameterException" in error_message
                and "Request has invalid parameters" in error_message
            ):
                return False
            ...
            print("Error comparing faces with Amazon:", e)
            failures += 1
            if failures > 10:
                return False
            continue
...
parser = argparse.ArgumentParser(description="Arc2Face eval for student embeddings.")
parser.add_argument(
    "--embeddings",
    default="../../attacks/transfer/val_frac.pkl",
    help="Path to embeddings pickle (e.g., val_frac.pkl, val_minus.pkl, test_partial.pkl).",
)
args = parser.parse_args()

# with open("./log/insight_student_embeddings_val_epoch4.pkl", "rb") as f:
...
# %%
os.makedirs("eval_arc2face_fracface", exist_ok=True)
levels = []
amazon_conf = []
amazon_success_at_5_list = []
amazon_success_rate_list = []
...
os.makedirs("disagree_examples", exist_ok=True)
for idx in range(1000):
    ...
    for i in range(1):
        ...
        # res_deepface = DeepFace.verify(np.array(images[i].convert("RGB")), np.array(Image.open(fn[idx]).convert("RGB")), enforce_detection=False, model_name="Facenet512")
        # deepface_pass.append(int(res_deepface["verified"]))
        # # deepface_sim.append(1-res_deepface["distance"])
        res = compare_face_insight(images[i], Image.open(fn[idx]))
        if res is not None:
            sim.append(res)
        amazon_pass = compare_face_amazon(Image.open(fn[idx]), images[i])
        amazon_conf.append(amazon_pass)
        amazon_passes.append(amazon_pass)
        if not amazon_pass and level == "1e-5":
            print("Disagree example found, saving...")
            hstack_images = hstack([Image.open(fn[idx]).convert("RGB").resize((160, 160)), images[i].resize((160, 160))])
            hstack_images.save(os.path.join("disagree_examples", f"idx_{idx}_img_{i}.png"))

    amazon_success_rate = float(np.mean(amazon_passes)) if amazon_passes else 0.0
    amazon_success_at_5 = int(any(amazon_passes))
    amazon_success_at_5_list.append(amazon_success_at_5)
    amazon_success_rate_list.append(amazon_success_rate)
    amazon_success_rate_avg = float(np.mean(amazon_success_rate_list))
    amazon_success_at_5_avg = float(np.mean(amazon_success_at_5_list))
    ...
    if len(face_1_boxes) == 0 or len(face_2_boxes) == 0:
        wandb.log({
            "at_least_one_passed_rate": np.mean(at_least_one_passed), 
                    ...
                    "amazon_conf": np.mean(amazon_conf) if amazon_conf else 0.0,
                    "amazon_success_rate": amazon_success_rate_avg,
                    "amazon_success_at_5": amazon_success_at_5_avg,
            })
        continue
    ...
    # generated_face = 
    real_face = Image.open(fn[idx]).convert("RGB")
    real_face = real_face.resize((160, 160))
    template = template.convert("RGB").resize((160, 160))
    
    for i in range(5):
        wandb.log({
            "image": wandb.Image(
                    hstack([real_face, template, images[i].resize((160, 160))]),
                )}) 
    wandb.log({
        "at_least_one_passed_rate": np.mean(at_least_one_passed), 
                "per_image_success_rate": success_rate,
                ...
                # # "deepface_pass_rate": np.mean(deepface_pass),
                # # "deepface_sim": np.mean(deepface_sim),
        })

# %%
np.unique(levels, return_counts=True)
with open("res_fracface.pkl", "wb") as f:
    pickle.dump({
        "levels": levels,
