...
with open("../../attacks/fracface/test_fairface.pkl", "rb") as f: 
...
for idx in range(1000):
    ...
    images[0].save(f"eval_arc2face/frac_{idx:04}_regen.png")
    ...
    # template.save(f"eval_arc2face/frac_{idx:04}_template.png")
    shutil.copy(fn[idx], f"eval_arc2face/frac_{idx:04}_real.png")
    cache = []
    face_1_boxes = []
    face_2_boxes = []
    for i in range(5):
    ...
    wandb.log({
        "at_least_one_passed_rate": np.mean(at_least_one_passed), 
                "per_image_success_rate": success_rate,
                "image": wandb.Image(hstack([real_face.resize((160, 160)), generated_face.resize((160, 160))])),
                "sim": np.mean(sim),
                "sim_high_only": np.mean([s for s in sim if s > 0.3]) if len([s for s in sim if s > 0.3]) > 0 else 0,
