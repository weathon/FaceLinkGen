...
# %% 
import wandb
wandb.init(project="arc2face_insight_eval", name="../log/insight_student_embeddings_val_epoch9.pkl")

# %%
import os
import shutil
os.makedirs("eval_arc2face", exist_ok=True)
levels = []
at_least_one_passed = []
sims = []
amazon_pass = []
for idx in range(1000):
    ...
    num_images = 1 
    images = pipeline(prompt_embeds=id_emb, num_inference_steps=25, guidance_scale=2.5, num_images_per_prompt=5).images
    images[0].save(f"eval_arc2face/minus_{idx:04}_regen.png")
    shutil.copy(fn[idx], f"eval_arc2face/minus_{idx:04}_real.png")
    continue
    cache = []
    face_1_boxes = []
    face_2_boxes = []
    for i in range(5):
        ...
        images[i].save(f"eval_arc2face_minusface/idx_{idx}_img_{i}.png")
        Image.open(fn[idx]).save(f"eval_arc2face_minusface/idx_{idx}_real.png")
    ...
    template = templates[idx].permute(1,2,0).numpy()
    ...
    print("Mean sim so far:", np.mean(sims) if len(sims) > 0 else 0)
# %%
np.unique(levels, return_counts=True)
import pickle
with open("res_minusface.pkl", "wb") as f:
    pickle.dump({
        "levels": levels,
