...
import pickle
with open("../results/holdout_original_embeddings.pkl", "rb") as f:
    id_embs = pickle.load(f)


id_embs["student_embeddings"] = id_embs["embeddings"]
...
for idx in range(1000):
    id_emb = torch.tensor(id_embs["student_embeddings"][idx][None,:]).to(torch.float16)
    ...
    face_2_boxes = []
    for i in range(5):
        res = compare_faces(images[i], Image.fromarray(id_embs["images"][idx][:,:,::-1]), "YOUR_FACEPP_API_KEY", "YOUR_FACEPP_API_SECRET")
        if len(res['faces2']) == 0:
        ...
        sim = compare_face_insight(images[i], Image.fromarray(id_embs["images"][idx][:,:,::-1]))
        if sim is not None:
            sims.append(sim)
    ...
    real_face = Image.fromarray(id_embs["images"][idx][:,:,::-1])
...
with open("res_original.pkl", "wb") as f:
    pickle.dump({ 
        "levels": levels,
