...
import os
import sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "..", "third_party", "tface", "recognition"))
...
class MinusBackbone(nn.Module):
    ...
    def forward(self, x):
        if self.mode == 'toy':
            ...
            x_residue_shuffle, _ = self.shuffle(x_residue_up)
            x_feature_ = self.recognizer(_)
        ...
        return x_encode, x_residue, x_feature, x_latent, x_residue_up, x_residue_shuffle, x_feature_
